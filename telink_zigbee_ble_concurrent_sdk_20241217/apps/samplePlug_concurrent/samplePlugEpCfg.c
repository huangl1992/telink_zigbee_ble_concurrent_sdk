/********************************************************************************************************
 * @file    samplePlugEpCfg.c
 *
 * @brief   This is the source file for samplePlugEpCfg
 *
 * @author  Zigbee Group
 * @date    2021
 *
...
#define ZCL_BASIC_MODEL_ID	   		{5,'T','L','S','R','9'}
#endif
...
#define COLOR_TEMPERATURE_DEFAULT		0x0176//2700K
...
zcl_lightColorCtrlAttr_t g_zcl_colorCtrlAttrs =
{
	...
	.colorTemperatureMireds			= COLOR_TEMPERATURE_DEFAULT,//COLOR_TEMPERATURE_PHYSICAL_MAX,
...
nv_sts_t zcl_onOffAttr_save(void)
{
	...
	if(needSave){
		...
		printf("ep 1: onoff = %x, startUp = %x\n", zcl_nv_onOff.onOff, zcl_nv_onOff.startUpOnOff);
...
nv_sts_t zcl_colorCtrlAttr_save(void)
{
	...
	if(st == NV_SUCC){
		if((zcl_nv_colorCtrl.currentHue != g_zcl_colorCtrlAttrs.currentHue) ||

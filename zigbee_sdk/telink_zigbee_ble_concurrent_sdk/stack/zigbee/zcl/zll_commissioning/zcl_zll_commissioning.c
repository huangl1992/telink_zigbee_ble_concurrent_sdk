...
_CODE_ZCL_ void	zcl_zllTouchLinkFinish(u8 status){
	...
	g_zb_txPowerSet = ldsZigbeeTxPowerSet();
...
//	printf("zllClientCmdRcv: id = %x, lqi = %x, lqiThreshold = %x\n", cmd, pApsdeInd->indInfo.lqi, g_zllTouchLink.lqiThreshold);
...
_CODE_ZCL_ void zcl_touchLinkStart(void){
	...
	g_zllTouchLink.commissionTxPower = ldsZigbeeTxPowerSet();

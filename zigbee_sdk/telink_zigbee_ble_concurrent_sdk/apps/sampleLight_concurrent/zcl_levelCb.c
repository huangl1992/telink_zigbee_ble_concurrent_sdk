...
bool ldsLightOnoffStatusGet(void)
{
	zcl_onOffAttr_t *onoff = zcl_onoffAttrGet();

	return onoff->onOff;
}

uint8_t ldsLightLevelGet(void)
{
	zcl_levelAttr_t *level = zcl_levelAttrGet();
...
static s32 sampleLight_levelTimerEvtCb(void * arg)
{
	...
	level_attribute_change_handle(0x0000, 0xFF, 0xFFFF, &pLevel->curLevel);
...
//	printf("sampleLight_moveToLevelProcess test2222: %d, %d, %d, %d\n", cmdId, levelInfo.stepLevel256, cmd->transitionTime, pLevel->remainingTime);
...
{
	//moveToLvl_t *test = (moveToLvl_t *)cmdPayload;
	//step_t *step_test = (step_t *)cmdPayload;

	ikea_sceneTimerStop();
	if(pAddrInfo->dstEp == SAMPLE_LIGHT_ENDPOINT){
		switch(cmdId){
			case ZCL_CMD_LEVEL_MOVE_TO_LEVEL:
				if (ldsLightOnoffStatusGet())
				{

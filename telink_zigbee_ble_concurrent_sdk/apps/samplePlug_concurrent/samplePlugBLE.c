/********************************************************************************************************
 * @file    samplePlugBLE.c
 *
 * @brief   This is the source file for samplePlugBLE
 *
 * @author  Zigbee Group
 * @date    2021
 *
...
typedef struct{
	u8 onOff; //BIT(0) for ep1; BIT(1) for ep2
}share_plug_t;
...
static void matter_prepare2Jump(void){
	...
	share_plug_t info_plug;
	memset((u8 *)&info_plug, 0xff, sizeof(share_plug_t));

	info_plug.onOff = 0;
	zcl_onOffAttr_t *pCurOnOffAttr = zcl_onoffAttrGet(SAMPLE_LIGHT_ENDPOINT);
	if(pCurOnOffAttr && pCurOnOffAttr->onOff){
		info_plug.onOff |= BIT(0);
	}
#if PLUG_EP_2_SUPPORT
	pCurOnOffAttr = zcl_onoffAttrGet(SAMPLE_LIGHT_ENDPOINT_2);
	if(pCurOnOffAttr && pCurOnOffAttr->onOff){
		info_plug.onOff |= BIT(1);
	}
#endif

	flash_write(JUMP2_MATTER_TAG_ADDR + 2, sizeof(share_plug_t), (u8 *)&info_plug);

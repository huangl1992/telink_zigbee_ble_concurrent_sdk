...
extern uint8_t g_scene_transtime;
/**********************************************************************
 * FUNCTIONS
 */
...
void sampleLight_colorInit(void)
{
	...
	printf("sampleLight_colorInit : %2x\n", pColor->colorTemperatureMireds);
}
...
uint16_t ldsLightUpdateColorMode(u8 colorMode)
{
	...
	switch (colorMode)
	{
		...
		case ZCL_COLOR_MODE_CURRENT_HUE_SATURATION:
			...
			break;
#endif
		case ZCL_COLOR_MODE_COLOR_TEMPERATURE_MIREDS:
			attribute_id = 0x0007;
			break;
...
void ldsColorAttributeChangeHandle(void)
{
	u16 value[2] = {0};
	uint16_t attribute_id = 0;
	u16 trans_time = 0;
	...
	attribute_id = ldsLightUpdateColorMode(pColor->colorMode);
	trans_time = g_scene_transtime * 100;

	if (pColor->colorMode == 0x02)
	{
		value[0] = pColor->colorTemperatureMireds;
	}
	else
	{
		value[0] = pColor->currentX;
		value[1] = pColor->currentY;
	}

	lds_light_control_set_transitionTime((uint32_t)trans_time);
	color_attribute_change_handle(attribute_id, 0xFF, 0xFFFF, (uint8_t *)value);
}
#else
void ldsColorAttributeChangeHandle(void)
{
	...
	else if ((pColor->enhancedColorMode == ZCL_COLOR_MODE_CURRENT_HUE_SATURATION)
			|| (pColor->colorMode == ZCL_COLOR_MODE_CURRENT_HUE_SATURATION))
	{
		attribute_id = 0x0000;
		value[0] = pColor->currentHue;
	}
	else if ((pColor->enhancedColorMode == ZCL_ENHANCED_COLOR_MODE_CURRENT_HUE_SATURATION)
			|| (pColor->colorMode == ZCL_ENHANCED_COLOR_MODE_CURRENT_HUE_SATURATION))
	{
		attribute_id = 0x0001;
		value[0] = pColor->currentSaturation;
	}
	else
	{
		value[0] = pColor->currentX;
...
//	printf("sampleLight_colorTimerEvtCb: %x, %x, %d, %d, %x\n", pColor->colorMode, pColor->enhancedColorMode,
//							pColor->currentX, pColor->currentY, pColor->colorTemperatureMireds);
...
#if (defined EXTENDEDCOLOR_LIGHT) || (defined COLORTEMPERATURE_LIGHT)
//#if COLOR_CCT_SUPPORT
/*********************************************************************
 ...
 * @return  None
 */
void sampleLight_moveToColorTemperatureProcess(zcl_colorCtrlMoveToColorTemperatureCmd_t *cmd)
{
	zcl_lightColorCtrlAttr_t *pColor = zcl_colorAttrGet();
...
static void sampleLight_stepColorTemperatureProcess(zcl_colorCtrlStepColorTemperatureCmd_t *cmd)
{
	...
	LDS_LOG_I("sampleLight_stepColorTemperatureProcess test1111: %d, %x, %d, %x, %x", pColor->colorMode, pColor->colorTemperatureMireds, cmd->stepMode,
			cmd->colorTempMaxMireds, cmd->colorTempMinMireds);
	...
	LDS_LOG_I("sampleLight_stepColorTemperatureProcess test3333: %d, %d, %d, %x, %x, %x", cmd->stepMode, colorInfo.colorTempRemainingTime, pColor->colorMode,
						pColor->colorTemperatureMireds, colorInfo.colorTempMinMireds, colorInfo.colorTempMaxMireds);

...
static s32 matterCommissionTimeoutCb(void *arg){
	...
	LDS_LOG_I("matter CMT = %d min\n", matterCommission.timeoutCnt);

	if(matterCommission.timeoutCnt == 0){
		LDS_LOG_I("ble adv stopped\n");
		ble_task_stop();
...
//	LDS_LOG_I("matter CMT[init] = %d min\n", matterCommission.timeoutCnt);
...
//			ota_queryStart(OTA_PERIODIC_QUERY_INTERVAL);
			...
			if (lds_trigger_join_network_flag_get() == 0xA5)
			{
 ...
 */
void zbdemo_bdbCommissioningCb(u8 status, void *arg){
	LDS_LOG_I("zbdemo_bdbCommissioningCb: sta = %d", status);

	switch(status){
		case BDB_COMMISSION_STA_SUCCESS:
			heartInterval = 1000;

			LDS_LOG_I("chn = %d, panId = %x\n", g_zbMacPib.phyChannelCur, g_zbNIB.panId);
#ifdef ZCL_OTA
	    	ota_queryStart(OTA_PERIODIC_QUERY_INTERVAL);
#endif
